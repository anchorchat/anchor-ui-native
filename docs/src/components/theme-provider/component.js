...
const ThemeProviderDoc = () => (
  <section className="page">
    ...
    {map(colors, (color, name) => <span><Text type="body-light" style={style.name}>{name}:</Text> <Text>{color}</Text></span>)}
    <h2><Text type="heading-secondary">Fonts</Text></h2>
    {map(fonts, (font, name) => <span><Text type="body-light" style={style.name}>{name}:</Text> <Text>fontFamily: {'{ '}{font.fontFamily}{' }'}</Text></span>)}

...
class MessageInput extends Component {
  state = {
    paddingBottom: null,
    keyboardOpen: false // eslint-disable-line react/no-unused-state
  }

  static getDerivedStateFromProps(props, state) {
    if (props.safeArea.bottom !== state.paddingBottom && !state.keyboardOpen) {
      return {
  ...
  keyboardWillShow = (event) => {
    ...
    this.setState({
      paddingBottom: 0,
      keyboardOpen: true // eslint-disable-line react/no-unused-state
    });
  };
